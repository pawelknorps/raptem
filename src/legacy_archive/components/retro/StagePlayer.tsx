...
interface SpektaklPlayerProps {
  audioUrl: string;
}

const SpektaklPlayer: React.FC<SpektaklPlayerProps> = ({ audioUrl }) => {
  ...
  return (
    <div className="teatr-player" style={{ textAlign: 'center', margin: '20px 0' }}>
      <button 
        className="win95-button" 
        onClick={togglePlayback}
        style={{ fontSize: '1.5rem', padding: '15px 30px', backgroundColor: '#ff00ff', color: 'white' }}
      >
        {isPlaying ? 'STOP SPEKTAKL' : 'KLIKNIJ ABY ODPALIĆ WIXĘ'}
      </button>
      <div style={{ marginTop: '10px', fontSize: '0.8rem', color: 'var(--text-lime)', fontFamily: 'var(--font-mono)' }}>
        {isPlaying ? '>>> SPEKTAKL IN PROGRESS <<<' : '--- SYSTEM READY ---'}
      </div>
    </div>
  );
};

export default SpektaklPlayer;
